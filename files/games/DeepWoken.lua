...
do -- // Functions
    ...
    function functions.BreakMobs(toggle)
        getgenv().BreakMobsFlower = toggle
    end
    ...
    RunService.Stepped:Connect(function()
        ...
        if Character and getgenv().BreakMobsFlower then
        ...
        if Character and (not getgenv().BreakMobsFlower) and not FixFling then
    ...
    RunService.Heartbeat:Connect(function()
        if getgenv().BreakMobsFlower then
            local Character = LocalPlayer.Character
            if Character then
                for i,v in Character:GetChildren() do
                    if v:IsA("BasePart") or v:IsA("Part")  then
        ...
        maid.streamerMode = RunService.Heartbeat:Connect(function()
            ...
            if (ultraStreamerMode) then
                for _, entity in next, workspace.Live:GetChildren() do
                    ...
                    humanoid.DisplayName = 'BUY Flower Hub';
                end;
            end;
    ...
    -- mudskipper big boy
    animTimes['17108258606'] = 0.4;
    animTimes['17108275299'] = 0.4;
    animTimes['17108566269'] = function(anim,mob) -- Jump
        repeat task.wait() until checkRange(25,mob.HumanoidRootPart) or not anim.IsPlaying
        if not anim.IsPlaying then return; end
        pingWait(.51);
        dodgeAttack();
    end;
    animTimes['17108826294'] = function(anim,mob) -- Slam
        repeat task.wait() until checkRange(25,mob.HumanoidRootPart) or not anim.IsPlaying
        if not anim.IsPlaying then return; end
        pingWait(.31);
        dodgeAttack();
    ...
    do -- // Auto Parry Helper
        if (not isfolder('Flower Hub/Block Points')) then
            makefolder('Flower Hub/Block Points');
        end;
            ...
            local function closestJar(isLayer2Pt2)
                ...
                for _, v in next, t do
                    ...
                    if (thing and (pos - myPos).magnitude < last) then
                        local meshPart = v:IsA('Model') and v:FindFirstChild('MeshPart');
    ...
    do -- Chunk Loader
        ...
        local function chunkFunction(v)
            ...
            if (v:IsA('Model') and FindFirstChildWhichIsA(v,"BasePart")) then
...
do -- // Local Cheats
    ...
    localCheats:AddToggle({
        ...
        tip = 'Breaks all auto parry other than users who are also using Flower Hub.',
